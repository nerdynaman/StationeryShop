...
def userMenu(mycursor,sessionID):
    ...
    #     else:
    #         mycursor.execute("select * from cartItem where SessionID = {}".format(sessionID))
    #         for cart in mycursor.fetchall():
    #             for elements in cart:
    #                 print(elements,end=" ")
    #             print()
    #         print()
    #         mycursor.execute("select sum(product.Cost*cartItem.quantity) from product,cartItem where product.ProductID = cartItem.ProductID and cartItem.SessionID = {}".format(sessionID))
    #         total = mycursor.fetchall()
    #         print("Total amount to be paid : ",total[0][0])
    #         mycursor.execute("delete from cartItem where SessionID = {}".format(sessionID))
    #         mydb.commit()
...
def adminMenu(mycursor):
    ...
    if choice == 1:
        ...
        mycursor.execute("select * from categoryd")
        ...
        mycursor.execute("select productid from (SELECT ProductID, SessionID FROM ((select distinct(SessionID) from cartitem) as s cross join (select ProductID from product) as pi) EXCEPT (SELECT ProductID,SessionID FROM cartitem)) as a")
        products = mycursor.fetchall()
        for product in products:
